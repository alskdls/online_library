...
const Cart = () => {
  ...
  if (!user) return <div style={{ padding: '20px', color: 'var(--text-main)' }}>Будь ласка, увійдіть.</div>;
  if (loading) return <div style={{ padding: '20px', color: 'var(--text-main)' }}>Завантаження...</div>;
    ...
    <div style={{ padding: '20px', color: 'var(--text-main)' }}>
      ...
      {cartItems.length > 0 && (
        <div style={summaryBoxStyle}>
          <h3 style={{ color: 'var(--text-main)' }}>Загальна сума: <span style={{ color: '#27ae60' }}>{calculateTotal()} грн</span></h3>
          <button 
